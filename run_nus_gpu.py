...
data_path = '/public/dataset/nus-wide/nus-wide.mat'
save_path = '/lqh/DeepUnsupervisedHashing/DSAH/data/output/nus-wide/' + version
logfile_dir = '/lqh/DeepUnsupervisedHashing/DSAH/data/log/nus-wide/'
logfile_name = version + '.log'

logger = setup_logger(logfile_name, logs_dir=logfile_dir, also_stdout=False)
logger.info('dataset: nus-wide')
...
train_data = np.array(h5py.File(data_path, mode='r')['train_data']).T
# todo: please providing the similarity matrix for training
W = scio.loadmat('/lqh/DeepUnsupervisedHashing/W/nus-wide/' + W + '.mat')['S']
...
def Save_binary_code(data_path, save_path, batch_size):
    mat = h5py.File(data_path, mode='r')
    dataset = np.array(mat['data_set']).T
    train = np.array(mat['train_data']).T  # database set
    test = np.array(mat['test_data']).T  # testdata set
    del mat
...
# 3.Evaluating
print('Evaluating...')
map_1000, map_5000, map_all = eval_map(temp_save_path, dataset='nus')
